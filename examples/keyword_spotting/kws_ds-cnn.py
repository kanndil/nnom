...
def train(x_train, y_train, x_test, y_test, type, batch_size=64, epochs=100):
    ...
    x = Conv2D(76, kernel_size=(5, 5), strides=(2, 1), padding='valid')(inputs)
    x = BatchNormalization()(x)
    x = ReLU()(x)

    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(3,3), padding="valid", activation="relu")(x)
    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(3,3), padding="valid", activation="relu")(x)
    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(2,2), padding="valid", activation="relu")(x)
    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(2,2), padding="valid", activation="relu")(x)
    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(2,2), padding="valid", activation="relu")(x)
    x = DepthwiseSeparableConv2D(filters=76, kernel_size=(2,2), padding="valid", activation="relu")(x)
...
def main():
    ...
    # do the job
    model, history = train(x_train, y_train, x_val, y_val, type=num_type, batch_size=batch_size, epochs=epochs)
